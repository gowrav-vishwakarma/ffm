...
class RokIcon
{
	function create($article, $params, $access, $attribs = array())
	{
		...
		$url = 'index.php?task=new&type=raw&ret='.base64_encode($ret).'&id=0&sectionid='.$article->sectionid;
	...
	{
		$url  = 'index.php?view=article&type=raw';
	...
	function email($article, $params, $access, $attribs = array())
	{
		...
		$url	= 'index.php?option=com_mailto&tmpl=component&type=raw&link='.base64_encode( $link );
	...
	function print_popup($article, $params, $access, $attribs = array())
	{
		...
		$url .=  @$article->catslug ? '&catid='.$article->catslug : '';
		$url .= '&id='.$article->slug.'&tmpl=component&type=raw&print=1&layout=default&page='.@ $request->limitstart;
