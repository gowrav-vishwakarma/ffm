...
function Node(id, pid, name, url, title, target, icon, iconOpen, open, details) {
...
function dTree(objName,imgPath) {
	...
	this.icon = {
		...
		middleLine		: imgPath+'myline2.gif',
		
		leftLine		: imgPath+'myline2.gif',
		
		smallLine		: imgPath+'smallLine.gif',
		
		rightLine		: imgPath+'myline2.gif'
...
dTree.prototype.add = function(id, pid, name, url, title, target, icon, iconOpen, open, details) {

	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title, target, icon, iconOpen, open, details);
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if (this.root.id == node.pid) {
			// For roon icon
			node.icon = node.icon; //this.icon.root;

			node.iconOpen = node.icon; //this.icon.root;
	...
	if (node.url) {
		...
		str += '<a id="s' + this.obj + nodeId + '" class=hasTip "' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="' + node.url + '"';

		if (node.title) str += ' xtitle="' + node.title + '"';

		if (node.target) str += ' target="' + node.target + '"';

		//str += ' onmouseover="javascript:return escape(\''+ node.details +'\');"  onmouseout="window.status=\'\';return true;" ';
