...
class View_Tree extends View{
	...
	function drawNode($parent_id,$id,$depth){
		...
		$this->js(true,"addNode($id,$parent_id,'".$m['name']." [".$m['inLeg']."]', '$clr', '".$this->createDetails($m)."')");
		if($m['left_id'] <> null)
			$this->drawNode($id,$m['left_id'],$depth-1);
		else if($depth-1 > 0)
			$this->js(true,"addNode(-${id}0001,$id,'A','question.gif','".$this->createDetails($m)."')");

		if($m['right_id'] <> null)
			$this->drawNode($id,$m['right_id'],$depth-1);
		else if($depth-1 > 0)
			$this->js(true,"addNode(-${id}0002,$id,'B','question.gif','".$this->createDetails($m)."')");

		$m->unload();
		$m->destroy();
	}

	function createDetails($m){
		$msg="";
		$sessiondetails=array();
		foreach($m->ref('Leg')->dsql()->order('Leg') as $junk){
			$sessiondetails[] = $junk['Leg']. "-". $junk['SessionPV'];
		}
		$msg =  "Session (". implode(',',$sessiondetails) .")";
		return $msg;
	}
