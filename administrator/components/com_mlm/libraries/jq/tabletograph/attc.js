...
jQuery(function($) {
	...
	$(".graph_table").each(function(i) {
		...
		swfobject.embedSWF(flash_chart_path, current_name, flash_chart_w,flash_chart_h, "9.0.0", "expressInstall.swf", flashvars, params, attributes );
...
function get_chart_type(table_el){
	var output='bar';
	if( jQuery(table_el).is(".chart_type_pie") ) {
    	output='pie';
	}
	if( jQuery(table_el).is(".chart_type_bar") ) {
    	output='bar';
	}
	if( jQuery(table_el).is(".chart_type_bar_3d") ) {
    	output='bar_3d';
	}
	if( jQuery(table_el).is(".chart_type_area") ) {
    	output='area';
	}
	if( jQuery(table_el).is(".chart_type_line") ) {
    	output='line';
	}
	return output;
}
function show_hide_chart(flash_chart,html_table){
	var current_name=jQuery(html_table).attr("id");
	jQuery(html_table).before('<div class="show_hide_container"><div class="flash_chart_title">'+jQuery(html_table).attr("summary")+'</div><a href="##" id="show_hide_flash_'+current_name+'" class="icon_link flash_chart_link">Show/hide chart</a><a href="##" id="show_hide_table_'+current_name+'" class="icon_link html_table_link">Show/hide table</a></div>');
	if (jQuery(html_table).is(".chart_hidden")) {
		jQuery(flash_chart).hide();
	}
	if (jQuery(html_table).is(".table_hidden")) {
		jQuery(html_table).hide();
	}
}
