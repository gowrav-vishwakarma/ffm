...
class DataMapper implements IteratorAggregate {
	...
	{
                if(!isset($GLOBALS['$xCICurrentExtension']))
                    $GLOBALS['$xCICurrentExtension']='';
	...
	public function DataMapper($id = NULL)
	{
		...
		if ( ! isset(DataMapper::$common[$common_key]))
		{
			...
			if(!empty($this->lang_file_format))
			{
				...
				if(file_exists(constant($xCICurrentExtension.'APPPATH').'language/'.$idiom.'/'.$lang_file.'_lang'.EXT))
				{
			...
			if( ! empty(DataMapper::$config['production_cache']))
			{
				...
				if (!is_dir($cache_folder = DataMapper::$config['production_cache']))
				{
					...
					$cache_folder = constant($xCICurrentExtension.'APPPATH') . DataMapper::$config['production_cache'];
				}
...
//	return false;
		...
		else
		{
			// search only the applications models folder
                    global $xCICurrentExtension;
			$paths[] = constant($xCICurrentExtension.'APPPATH');
		}
	...
	protected static function _load_extensions(&$extensions, $names)
	{
		...
		foreach($names as $name => $options)
		{
			...
			{
				$file = constant($xCICurrentExtension.'APPPATH') . DataMapper::$config['extensions_path'] . '/' . $name . EXT;
				$ext = $name;
			}
			else
			{
				$file = constant($xCICurrentExtension.'APPPATH') . $name . EXT;
